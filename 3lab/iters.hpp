...
void libman(const std::vector<std::vector<double>>& matrix, std::vector<double> betha, std::vector<double>& ans, double omega, double eps){
  ...
  for(unsigned int iter = 0; iter<1000000; ++iter){
    ...
    if(std::pow(a, iter+1)/(1.0 - a)*norm(ans-buff) <= eps){
...
void zeldel(const std::vector<std::vector<double>>& matrix, std::vector<double> betha, std::vector<double>& ans, double eps){
  ...
  double a=norm(alpha);
  if(a>=1)
    std::cerr<<"Warning: ||alpha|| >= 1"<<std::endl;
  for(unsigned int iter = 0; iter<1000000; ++iter){
    for(unsigned int i = 0; i<n; ++i){
      ans[i] = prod(alpha[i], ans) + betha[i];
    }
    if(std::pow(a, iter+1)/(1.0 - a)*norm(ans-prev) <= eps){
      std::cout<<"Iter finish:"<<iter<<std::endl;
      break;
    }
    prev=ans;
  } 
